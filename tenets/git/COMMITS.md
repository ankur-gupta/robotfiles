# Global Git Commit Tenets [GLOBAL-GIT-COMMIT-TENETS]
This file defines global Git commit-message tenets (in [TENETS] section) for LLM coding agents. Project-local instructions may override it when they are more specific.

## Using This File [USING-THIS-FILE]
This [GLOBAL-GIT-COMMIT-TENETS] file is part of a hierarchy
  1. Project-specific Git or commit guide (if it exists)
  2. This [GLOBAL-GIT-COMMIT-TENETS] file
  3. General Git commit-message conventions

If there is a conflict between the above three sources, the project-specific Git or commit guide supersedes this [GLOBAL-GIT-COMMIT-TENETS] file, which then supersedes general Git commit-message conventions.

### Finding tenets [FINDING-TENETS]
- When you see a tenet citation like "Tenet X [STABLE-ID]", give precedence to the semantic text of stable ID over the number when trying to find the relevant tenet. Numbering can get messed up.
- When explaining a change, cite the relevant tenet only when it clarifies the reasoning.
- Always cite a tenet in this form: "Tenet X [STABLE-ID]". You may cite the tenet in commit messages, codebase comments, other markdown files, or even this file itself.
...
- Anti-patterns are discouraged but allowed in this [GLOBAL-GIT-COMMIT-TENETS] file in the [EXCEPTIONS] section.
